(function(){
  ...
  function updateLatexLineNumbersAssistant(){
    ...
    // ensure numbers stay inside the pre and align with textarea lines
    ln.style.whiteSpace = 'pre';
    ln.style.textAlign = 'right';
    ln.style.paddingRight = '8px';
    ln.style.overflow = 'auto';
    try { ln.style.lineHeight = window.getComputedStyle(ta).lineHeight || '20px'; } catch(e){}
    ln.style.width = (ln.style.width || '45px');
    ...
    const code = ta.value || '';
    if (code.trim()){
      // Use textContent to avoid HTML parsing and preserve spaces; MathJax will read the math delimiters.
...
(function(){
  ...
  // simple watcher to detect data changes and auto-refresh relevant UI (handles external edits or teacher actions)
  (function(){
    let snapshot = JSON.stringify({ g:(window.appData.grades||[]).length, q:(window.appData.quizzes||[]).length, s:(window.appData.slides||[]).length, m:(window.appData.messages||[]).length, l:(window.appData.latexContents||[]).length });
    setInterval(function(){
      try {
        const raw = localStorage.getItem('edu_app_data_v1') || '{}';
        const parsed = JSON.parse(raw);
        window.appData = Object.assign({}, parsed, window.appData); // merge persisted data
        const currentSnap = JSON.stringify({ g:(window.appData.grades||[]).length, q:(window.appData.quizzes||[]).length, s:(window.appData.slides||[]).length, m:(window.appData.messages||[]).length, l:(window.appData.latexContents||[]).length });
        if (currentSnap !== snapshot){
          snapshot = currentSnap;
          // refresh UI pieces
          try { renderStudentDashboard(); renderSliderAdminList(); renderFrontSlider(); populateRevisionSelect(); renderStudentRevisionRequests(); renderRevisionRequestsList(); renderLatexListForStudents(); } catch(e){ console.error(e); }
        }
      } catch(e){}
    }, 1500);
  })();

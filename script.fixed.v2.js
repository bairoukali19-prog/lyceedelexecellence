...
/* =============================
   ...
   ============================= */  
const STORAGE_KEY = 'lyceeExcellence_v_17';
let appData = {
  students: [
...
// =============================
// Deep defensive handler to stop "jump to top" when clicking inside sections
// - Captures clicks in capture phase so it runs before other handlers
// - Prevents default for anchors with empty/hash hrefs that don't map to an element
// - Ensures buttons inside sections don't submit forms unintentionally
// - Uses MutationObserver to re-normalize dynamically added buttons/anchors
// Inserted by ChatGPT fixer
// =============================
(function(){
  function isHashOnly(h){
    if(!h) return true;
    h = h.trim();
    return h === '' || h === '#' || h === '#!' ;
  }

  function safeAnchorCheck(a){
    try{
      ...
      return true; // non-hash link is safe (external or normal)
  ...
  document.addEventListener('click', function(ev){
    try{
      ...
      if(!el) return;
      // only enforce within a section element (user reported problem)
      const inSection = !!ev.target.closest('section');
      if(!inSection) return;

      if(el.tagName === 'A'){
        // if anchor points to a non-existent fragment or is just '#', prevent default
        const href = el.getAttribute('href') || '';
        if(!safeAnchorCheck(el)){
          ev.preventDefault();
          ev.stopPropagation();
          return;
        }
      ...
      if(el.tagName === 'BUTTON' || (el.tagName === 'INPUT' && (el.type === 'submit' || el.type === 'image'))){
        // determine type (buttons default to 'submit' if not specified)
        const hasTypeAttr = el.hasAttribute && el.hasAttribute('type');
        const type = (hasTypeAttr ? (el.getAttribute('type')||'').toLowerCase() : (el.tagName === 'BUTTON' ? 'submit' : el.type||''));
        // if button is not explicitly a submitter, prevent default to avoid form submit
        if(type !== 'submit' && type !== 'image'){
          ev.preventDefault();
          ev.stopPropagation();
          return;
        }
        // if it is a submit, check the form action to avoid submitting to '#' or empty action
      ...
      // swallow errors; do not break other handlers
      console.warn('deep click handler error', err);
    }
  }, true); // capture phase

  // MutationObserver to re-normalize buttons/anchors that are added dynamically
  const mo = new MutationObserver(function(mutations){
    try{
      mutations.forEach(m => {
        m.addedNodes && m.addedNodes.forEach(node => {
          if(!(node instanceof HTMLElement)) return;
          // set default type on buttons
          node.querySelectorAll && node.querySelectorAll('button').forEach(b => { if(!b.hasAttribute('type')) b.setAttribute('type','button'); });
          // neutralize href="#" anchors
          node.querySelectorAll && node.querySelectorAll('a[href="#"], a[href="#!"]').forEach(a => {
            if(!a.__preventedHash){ a.addEventListener('click', e => e.preventDefault()); a.__preventedHash = true; }
          });
        });
      });
    }catch(err){ console.warn('mutation observer normalization error', err); }
  });

...
/* =============================
   ...
   ============================= */
function wireEvents(){
  // منع التمرير لجميع الأزرار
  document.addEventListener('click', function(e) {
    if (e.target.tagName === 'BUTTON' || (e.target.tagName === 'A' && e.target.classList.contains('nav-link'))) {
      e.preventDefault();
      e.stopPropagation();
    }
  }, true);
  ...
  // البحث المتأخر عن الأزرار المفقودة
  setTimeout(() => {
    // البحث عن زر إضافة القاموس
    const dictButtons = document.querySelectorAll('button');
    ...
    // البحث عن زر عرض النتائج
    const gradeButtons = document.querySelectorAll('button');
    gradeButtons.forEach(btn => {
      if (btn.textContent.includes('Voir les notes') || btn.textContent.includes('View grades') || btn.textContent.includes('عرض النتائج')) {
        if (!btn.hasAttribute('data-wired')) {
          btn.setAttribute('data-wired', 'true');
          btn.addEventListener('click', e => {
            e.preventDefault();
...
function hideAllMainSections(){
  ...
  if ($('student-dashboard') && $('student-dashboard').style) $('student-dashboard').style.display='none'; 
  if ($('admin-panel') && $('admin-panel').style) $('admin-panel').style.display='none';
}
...
function showSection(id){
  ...
  // إذا لم يكن هناك مستخدم مسجل، إظهار الصفحة الرئيسية فقط
  if (!appData.currentUser && id !== 'home') {
    if ($('home-section') && $('home-section').style) {
      $('home-section').style.display='block';
    }
    return;
  }
  
  // تحميل المحتوى المناسب لكل قسم
  ...
  if (id === 'grades') renderGradesSection();
}

function switchStudentTab(tabName){
  if (!tabName) return;
  
  document.querySelectorAll('.student-tab-content').forEach(x => {
...
function switchAdminTab(tabName){
  ...
  document.querySelectorAll('.admin-tab-link').forEach(l => {
...
function refreshUI(){
  ...
  // التحكم في عرض لوحة المسؤول والطالب
  if ($('admin-panel')) {
  ...
  // إذا لم يكن هناك مستخدم مسجل، إظهار الصفحة الرئيسية
  if (!appData.currentUser) {
    ...
    // إذا كان هناك مستخدم مسجل، إظهار القسم المناسب
    if (appData.isAdmin) {
...
/* =============================
   Grades search functionality - COMPLETELY FIXED
   ============================= */
...
function renderGradesSection(){
  ...
  if (btnViewGrades) {
    btnViewGrades.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      const code = document.getElementById('gradeSearchCode').value.trim();
      searchGradesByCode(code);
    });
  }
}
...
function adminSaveStudent(){
  ...
  if (!fullname || !username) {
    alert('Nom complet و nom d\'utilisateur requis');
    return;
  }
  ...
  if (existing) { 
    ...
    appData.students.push({ id, fullname, username, password, code, classroom });
  }
...
function loadStudentsTable(){
  ...
  appData.students.forEach(s => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button data-id="${s.id}" class="edit-student">Edit</button> 
        <button data-id="${s.id}" class="del-student">Del</button>
      </td>
    `;
    tbody.appendChild(tr);
  });
  
  tbody.querySelectorAll('.edit-student').forEach(b => {
    b.addEventListener('click', e => {
      ...
      if ($('stId')) $('stId').value=s.id; 
      if ($('stFullname')) $('stFullname').value=s.fullname; 
      if ($('stUsername')) $('stUsername').value=s.username; 
      if ($('stPassword')) $('stPassword').value=s.password; 
      if ($('stCode')) $('stCode').value=s.code; 
      if ($('stClassroom')) $('stClassroom').value=s.classroom; 
      switchAdminTab('tab-students');
    });
  });
  
  tbody.querySelectorAll('.del-student').forEach(b => {
    b.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      const id = b.getAttribute('data-id'); 
...
function populateStudentsSelect(){
  ...
  appData.students.forEach(s => { 
    ...
    o.value = s.id; 
    o.textContent = s.fullname + ' (' + (s.code||'') + ')'; 
    sel.appendChild(o); 
  });
  
  const gr = $('grStudent'); 
  if (gr){ 
    gr.innerHTML='<option value="">-- Choisir étudiant --</option>'; 
    appData.students.forEach(s => { 
      const o = document.createElement('option'); 
      o.value=s.id; 
  ...
  const gf = $('grFilterStudent'); 
  if (gf){ 
    gf.innerHTML='<option value="">Tous</option>'; 
    appData.students.forEach(s => { 
      const o = document.createElement('option'); 
      o.value=s.id; 
      o.textContent=s.fullname; 
...
function adminSaveGrade(){
  ...
  if (!studentId || !subject || !title) {
    alert('Remplir étudiant، matière و intitulé');
    return;
  }
...
function loadGradesTable(){
  ...
  appData.grades.forEach(g => {
    ...
    tr.innerHTML = `
      ...
      <td><button data-id="${g.id}" class="del-grade">Del</button></td>
    `;
    tbody.appendChild(tr);
  });
  
  tbody.querySelectorAll('.del-grade').forEach(b => {
    b.addEventListener('click', e => { 
      e.preventDefault();
      e.stopPropagation();
      const id = b.getAttribute('data-id'); 
      if (!confirm('Supprimer note ?')) return; 
...
function adminSendMessage(){
  ...
  if (!title || !content) {
    alert('Titre و contenu requis');
    return;
  }
  
  if (target === 'specific'){ 
    const sid = $('adminMessageStudent').value; 
    if (!sid) {
      alert('Choisir طالب'); 
      return;
    } 
    appData.messages.push({ id: genId(), title, content, target:'specific', specific:sid, createdAt:Date.now() }); 
  } else {
...
function renderAdminMessagesList(){
  const c = $('adminMessagesList'); 
  if (!c) return; 
  
  c.innerHTML='';
  
  if (!appData.messages.length) {
    c.innerHTML='<p class="muted">لا توجد رسائل</p>';
    return;
  }
  
  appData.messages.forEach(m => { 
    const d = document.createElement('div'); 
    d.textContent='['+new Date(m.createdAt).toLocaleString()+'] '+m.title+' - '+m.content+' (cible:'+m.target+')'; 
    const del = document.createElement('button'); 
    del.textContent='Supprimer'; 
    del.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if (!confirm('Supprimer message ?')) return; 
      appData.messages = appData.messages.filter(x=>x.id!==m.id); 
      saveData(); 
      renderAdminMessagesList(); 
      renderStudentMessages(); 
    }); 
...
function renderStudentMessages(){
  const c = $('studentMessagesList'); 
  if (!c) return; 
  
  c.innerHTML='';
  
  if (!appData.currentUser) {
    c.innerHTML='<p class="muted">Connectez-vous</p>';
    return;
  }
  ...
  if (!list.length) {
    c.innerHTML='<p class="muted">لا توجد رسائل حتى الآن.</p>';
    return;
  }
  
  list.forEach(m => { 
    const d = document.createElement('div'); 
    d.textContent='['+new Date(m.createdAt).toLocaleString()+'] '+m.title+' - '+m.content; 
    c.appendChild(d); 
  });
}
...
function adminCreateQuiz(){
  ...
  const duration = Number($('newQuizDuration').value) || 0; 
  const shuffle = !!$('newQuizShuffle').checked; 
  const multiAttempts = !!$('newQuizAllowMultipleAttempts').checked;
  ...
  notifyStudents('quiz','Quiz جديد متاح','تم إضافة Quiz جديد. راجع قسم Quiz في Tableau de bord.'); 
  
  if ($('newQuizTitle')) $('newQuizTitle').value=''; 
  if ($('newQuizDuration')) $('newQuizDuration').value='0'; 
  if ($('newQuizShuffle')) $('newQuizShuffle').checked=false; 
  if ($('newQuizAllowMultipleAttempts')) $('newQuizAllowMultipleAttempts').checked=false;
}

function populateAdminSelectQuiz(){
  const sel = $('adminSelectQuiz'); 
  if (!sel) return; 
  
  sel.innerHTML=''; 
  const opt = document.createElement('option'); 
  opt.value=''; 
  opt.textContent='-- Sélectionner quiz --'; 
  sel.appendChild(opt);
  
  appData.quizzes.forEach(q => { 
    const o = document.createElement('option'); 
    o.value=q.id; 
    o.textContent=q.title + ' ('+q.questions.length+' q)'; 
    sel.appendChild(o); 
  });
}
...
function adminAddQuestion(){
  ...
  if (!qid) {
    alert('Sélectionner quiz');
    return;
  }
  
  const quiz = appData.quizzes.find(x=>x.id===qid); 
  if (!quiz) return;
  
  const questionText = $('adminQuizQuestion') ? $('adminQuizQuestion').value.trim() : '';
  
  if (!questionText) {
    alert('Question requise');
    return;
  }
  
  const type = $('adminQuestionType') ? $('adminQuestionType').value : 'single'; 
  const points = $('adminQuestionPoints') ? Number($('adminQuestionPoints').value) || 1 : 1;
  ...
  if (fileInput && fileInput.files && fileInput.files[0]) {
    ...
    fr.onload = function(ev){
      ...
      renderQuizAdminListDetailed(); 
      renderQuizList(); 
      alert('Question ajoutée avec image');
    };
    fr.readAsDataURL(f);
  } else {
  ...
  // مسح الحقول
  ['adminQuizQuestion','adminOption1','adminOption2','adminOption3','adminOption4','adminOption5','adminOption6','adminQuizCorrect','adminQuestionFeedback','adminQuizImage'].forEach(id => { 
    if ($(id)) { 
      try{ 
        $(id).value=''; 
      } catch(e){} 
...
function renderQuizAdminListDetailed(){
  const el = $('quizQuestionsList'); 
  if (!el) return; 
  
  el.innerHTML='';
  
  if (!appData.quizzes.length) { 
    if ($('adminSelectQuiz')) populateAdminSelectQuiz(); 
    if (el) el.innerHTML = '<p class="muted">Aucun quiz</p>'; 
    return; 
  }
  ...
  appData.quizzes.forEach(q => {
    ...
    header.innerHTML = '<strong>'+escapeHtml(q.title)+'</strong> (durée: '+q.durationMinutes+'m, shuffle:'+ (q.shuffle?'oui':'non') +', attempts:'+(q.allowMultipleAttempts?'yes':'no')+')';
    ...
    q.questions.forEach((qq, idx) => {
      ...
      qq.options.forEach((op,i) => {
        ...
        opLi.textContent = (i+1)+'. '+op + (qq.correctIndices.includes(i) ? ' (✓ correct)' : '');
      ...
      if (qq.feedback) { 
        ...
        fb.textContent = 'Feedback: '+qq.feedback; 
        li.appendChild(fb); 
      }
      ...
      btnUp.addEventListener('click', e => {
        ...
        e.stopPropagation();
        if (idx===0) return; 
        q.questions.splice(idx-1,0,q.questions.splice(idx,1)[0]); 
        saveData(); 
        renderQuizAdminListDetailed(); 
      });
      ...
      btnDown.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        if (idx===q.questions.length-1) return; 
        q.questions.splice(idx+1,0,q.questions.splice(idx,1)[0]); 
        saveData(); 
        renderQuizAdminListDetailed(); 
      });
      ...
      btnDel.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        if (!confirm('Supprimer question ?')) return; 
        q.questions = q.questions.filter(x=>x.id!==qq.id); 
        saveData(); 
        renderQuizAdminListDetailed(); 
      });
    ...
    const btnDelQuiz = document.createElement('button'); 
    btnDelQuiz.textContent='Supprimer quiz entier'; 
    btnDelQuiz.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if (!confirm('Supprimer quiz entier ?')) return; 
      appData.quizzes = appData.quizzes.filter(x=>x.id!==q.id); 
      saveData(); 
      renderQuizAdminListDetailed(); 
      renderQuizList(); 
    });
...
function renderQuizList(){
  ...
  if (!appData.quizzes.length) {
    c.innerHTML = '<p class="muted">Aucun quiz disponible pour le moment.</p>';
    return;
  }
  
  appData.quizzes.forEach(q => {
    const d = document.createElement('div'); 
    d.innerHTML = '<h3>'+escapeHtml(q.title)+'</h3><p>'+q.questions.length+' questions</p>';
    const btn = document.createElement('button'); 
    btn.textContent='Voir / Démarrer'; 
    btn.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if (!appData.currentUser){ 
...
function renderQuizListForStudent(){
  const c = $('studentQuizList'); 
  if (!c) return; 
  
  c.innerHTML='';
  
  if (!appData.quizzes.length) {
    c.innerHTML='<p class="muted">Aucun quiz</p>';
    return;
  }
  
  appData.quizzes.forEach(q => {
    const d = document.createElement('div'); 
    d.innerHTML = '<strong>'+escapeHtml(q.title)+'</strong> — '+q.questions.length+' q';
...
function startQuiz(quizId){
  const quiz = appData.quizzes.find(x=>x.id===quizId); 
  if (!quiz) {
  ...
  if (currentRun.timeLeft > 0){
    currentRun.timerInterval = setInterval(() => { 
      currentRun.timeLeft--; 
      renderTimer(); 
      if (currentRun.timeLeft<=0){ 
        clearInterval(currentRun.timerInterval); 
        alert('Temps écoulé'); 
        submitCurrentQuiz(); 
      } 
    }, 1000);
...
function renderTimer(){
  const area = $('quizTimer'); 
  if (area && currentRun) {
    area.textContent = formatTime(currentRun.timeLeft);
  }
}
...
function renderQuizRunner(){
  ...
  const header = document.createElement('div'); 
  header.innerHTML = '<h3>'+escapeHtml(quiz.title)+' — Question '+(currentRun.pos+1)+'/'+currentRun.order.length+'</h3>';
  
  if (quiz.durationMinutes) {
    header.innerHTML += '<div id="quizTimer">Temps restant: '+formatTime(currentRun.timeLeft)+'</div>';
  }
  
  container.appendChild(header);
  
  const qdiv = document.createElement('div'); 
  qdiv.innerHTML = '<p>'+escapeHtml(qobj.question)+'</p>'; 
  container.appendChild(qdiv);
  
  if (qobj.imageData){ 
    const img = document.createElement('img'); 
    img.src=qobj.imageData; 
  ...
  qobj.options.forEach((opt,i) => {
    ...
    if (qobj.type === 'single') {
      if (userResp === i) { 
        ...
        optEl.style.background='#e0e0e0'; 
      }
    } else {
      if (Array.isArray(userResp) && userResp.includes(i)) { 
        optEl.style.fontWeight='700'; 
        optEl.style.background='#e0e0e0'; 
      }
    }
  ...
  btnPrev.disabled = currentRun.pos === 0; 
  btnPrev.addEventListener('click', e => {
    e.preventDefault();
    e.stopPropagation();
    currentRun.pos = Math.max(0, currentRun.pos-1); 
    renderQuizRunner(); 
  });
  
  const btnNext = document.createElement('button'); 
  btnNext.textContent='Suivant'; 
  btnNext.disabled = currentRun.pos === currentRun.order.length-1; 
  btnNext.addEventListener('click', e => {
    e.preventDefault();
    e.stopPropagation();
    currentRun.pos = Math.min(currentRun.order.length-1, currentRun.pos+1); 
    renderQuizRunner(); 
  });
  
  const btnSubmit = document.createElement('button'); 
  btnSubmit.textContent='Soumettre le quiz'; 
...
function submitCurrentQuiz(){
  ...
  quiz.questions.forEach(q => {
    ...
    if (q.type === 'single'){
      if (user !== undefined && q.correctIndices.includes(user)) {
      ...
      if (userArr.length && userArr.length === correctArr.length && userArr.every((v,i)=>v===correctArr[i])) {
        gotPoints += q.points || 1;
      }
    }
  });
...
function showQuizResultsForStudent(quizId, responses){
  ...
  title.textContent = 'Résultats: ' + quiz.title; 
  ...
  quiz.questions.forEach((q,idx) => {
    ...
    let userStr = '';
    if (q.type === 'single'){ 
      userStr = (user === undefined ? 'Aucune réponse' : String.fromCharCode(65+user)); 
    } else if (Array.isArray(user) && user.length) {
      userStr = user.map(i=>String.fromCharCode(65+i)).join(', '); 
    } else { 
      userStr = 'Aucune réponse'; 
    }
    ...
    if (q.feedback) {
      block.innerHTML += '<div>Explication: '+escapeHtml(q.feedback)+'</div>';
    }
...
function adminSaveLesson(){ 
  if (!$('adminLessonTitle')) return; 
  
  const title = $('adminLessonTitle').value.trim();
  const link = $('adminLessonDriveLink').value.trim();
  
  if(!title||!link) {
    alert('Titre و lien requis');
    return;
  }
  
  appData.lessons.push({id:genId(),title,driveLink:link});
  saveData(); 
  renderLessons(); 
  renderLessonsAdminList(); 
  alert('Leçon ajoutée'); 
  
  if ($('adminLessonTitle')) $('adminLessonTitle').value=''; 
  if ($('adminLessonDriveLink')) $('adminLessonDriveLink').value=''; 
}

function renderLessons(){ 
  const c = $('lessonsContent'); 
  if(!c) return; 
  
  c.innerHTML=''; 
  
  if(!appData.lessons.length) {
    c.innerHTML='<p class="muted">Aucune leçon</p>';
    return;
  }
...
function renderLessonsAdminList(){ 
  ...
  appData.lessons.forEach(l => { 
    const d = document.createElement('div'); 
    d.innerHTML = escapeHtml(l.title) + ' '; 
    ...
    del.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if(!confirm('Supprimer leçon ?')) return; 
      appData.lessons = appData.lessons.filter(x=>x.id!==l.id); 
      saveData(); 
      renderLessonsAdminList(); 
      renderLessons(); 
    }); 
...
function adminSaveExercise(){ 
  if (!$('adminExerciseTitle')) return; 
  
  const title = $('adminExerciseTitle').value.trim();
  const link = $('adminExerciseDriveLink').value.trim();
  ...
  appData.exercises.push({id:genId(),title,driveLink:link});
  saveData(); 
  renderExercises(); 
  renderExercisesAdminList(); 
  alert('Exercice ajouté'); 
  
  if ($('adminExerciseTitle')) $('adminExerciseTitle').value=''; 
  if ($('adminExerciseDriveLink')) $('adminExerciseDriveLink').value=''; 
}

function renderExercises(){ 
  const c = $('exercisesContent'); 
  if(!c) return; 
  
  c.innerHTML=''; 
  
  if(!appData.exercises.length) {
    c.innerHTML='<p class="muted">Aucun exercice</p>';
    return;
  }
  
  appData.exercises.forEach(e => { 
    const d = document.createElement('div'); 
    const a = document.createElement('a'); 
    a.href=e.driveLink; 
    a.target='_blank'; 
...
function renderExercisesAdminList(){ 
  ...
  appData.exercises.forEach(e => { 
    const d = document.createElement('div'); 
    d.innerHTML = escapeHtml(e.title) + ' '; 
    ...
    del.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if(!confirm('Supprimer exercice ?')) return; 
      appData.exercises = appData.exercises.filter(x=>x.id!==e.id); 
      saveData(); 
      renderExercisesAdminList(); 
      renderExercises(); 
    }); 
...
function adminSaveExam(){ 
  if (!$('adminExamTitle')) return; 
  
  const title = $('adminExamTitle').value.trim();
  const link = $('adminExamDriveLink').value.trim();
  ...
  appData.exams.push({id:genId(),title,driveLink:link});
  saveData(); 
  renderExams(); 
  renderExamsAdminList(); 
  alert('Examen ajouté'); 
  
  if ($('adminExamTitle')) $('adminExamTitle').value=''; 
  if ($('adminExamDriveLink')) $('adminExamDriveLink').value=''; 
}
...
function renderExams(){ 
  ...
  if(!appData.exams.length) {
    c.innerHTML='<p class="muted">Aucun examen</p>';
    return;
  }
  ...
  appData.exams.forEach(e => {
    ...
    if (appData.currentUser && !appData.isAdmin){
      ...
      reqBtn.textContent='طلب إعادة تصحيح';
...
function renderExamsAdminList(){ 
  ...
  appData.exams.forEach(e => { 
    ...
    del.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if(!confirm('Supprimer examen ?')) return; 
      appData.exams=appData.exams.filter(x=>x.id!==e.id); 
      saveData(); 
      renderExamsAdminList(); 
      renderExams(); 
    }); 
...
function renderRegradeRequestsAdminList(){
  ...
  list.forEach(req => {
    ...
    markDone.textContent = req.handled ? 'معالج' : 'وضع كمُعالَج'; 
...
function handleAnnouncementImage(e){
  ...
  const fr = new FileReader(); 
  fr.onload = function(ev){ 
    appData.announcement.image = ev.target.result; 
    if ($('announcementImagePreview')) { 
      $('announcementImagePreview').src = ev.target.result; 
      $('announcementImagePreview').style.display='block'; 
    } 
    if ($('announcementImage')) { 
      $('announcementImage').src = ev.target.result; 
      $('announcementImage').style.display='block'; 
    } 
    if ($('btnDeleteAnnouncementImage')) $('btnDeleteAnnouncementImage').style.display='inline-block'; 
    saveData(); 
  ...
  if (btn) {
    btn.addEventListener('click', e => {
      ...
      if (url) {
        ...
        alert('Choisir une image ou fournir URL');
      }
...
function loadStudentDashboard(){
  ...
  if (recentEl) {
    ...
    if (!grades.length) {
      recentEl.innerHTML = '<p class="muted">Aucune note disponible pour le moment.</p>';
    } else {
  ...
  if (quizEl) {
    ...
    if (!list.length) {
      quizEl.innerHTML = '<p class="muted">Aucun quiz disponible pour le moment.</p>';
    } else {
      quizEl.innerHTML = '<ul>' + list.map(q=>'<li>'+ escapeHtml(q.title) +' ('+ q.count +' questions)</li>').join('') + '</ul>';
    }
  }
...
function adminSaveLatex(){
  ...
  const desc = $('latexDescription').value; 
  const cat = $('latexCategory').value;
  
  if (!title || !code) {
    alert('Titre و code requis');
    return;
  }
  
  appData.latexContents.push({ id: genId(), title, code, description: desc, category: cat, createdAt: Date.now() });
  saveData(); 
  alert('Leçon LaTeX محفوظة'); 
  loadLatexAdminList(); 
  renderLatexListForStudents();
  
  if ($('latexTitle')) $('latexTitle').value=''; 
  if ($('latexCode')) $('latexCode').value=''; 
  if ($('latexDescription')) $('latexDescription').value=''; 
  if ($('latexCategory')) $('latexCategory').value=''; 
  
  updateLatexLineNumbers();
}

function loadLatexAdminList(){
  const c = $('latexContentsList'); 
  if (!c) return; 
  
  c.innerHTML='';
  
  if (!appData.latexContents.length) {
    c.innerHTML = '<p class="muted">لا توجد دروس حتى الآن.</p>';
    return;
  }
  
  appData.latexContents.forEach(item => {
    const d = document.createElement('div'); 
    d.innerHTML = '<strong>' + escapeHtml(item.title) + '</strong> — ' + escapeHtml(item.description || '');
    ...
    btnDel.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if(!confirm('Supprimer الدرس ؟')) return; 
      appData.latexContents = appData.latexContents.filter(x=>x.id!==item.id); 
      saveData(); 
      loadLatexAdminList(); 
      renderLatexListForStudents(); 
    });
    
    d.appendChild(btnPreview); 
...
function renderLatexListForStudents(){
  const c = $('studentCoursList'); 
  if (!c) return; 
  
  c.innerHTML='';
  
  if (!appData.latexContents.length) {
    c.innerHTML='<p class="muted">لا توجد دروس</p>';
    return;
  }
  ...
  appData.latexContents.forEach(item => {
    ...
    btn.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      const target = $('quizContent'); 
      if (!target) return;
      target.innerHTML = '<h3>' + escapeHtml(item.title) + '</h3><div id="studentLatexArea">\\[' + item.code + '\\]</div>';
      if (window.MathJax) {
        MathJax.typesetPromise([document.getElementById('studentLatexArea')]).catch(()=>{});
      }
    });
...
function renderDictionary(){
  const el = $('dictionaryContent'); 
  if (!el) return; 
  
  el.innerHTML='';
  
  if (!appData.dictionary.length) {
    el.innerHTML = '<p class="muted">Aucun terme dans le lexique pour le moment.</p>';
    return;
  }
